...
public function getsupplierdata()
    ...
    public function ledger_report($data)
    {
      $ledgerdata=" select * from(
            select 'Expense Voucher'as types,convert(jm_expensemaster.InvDate,date)as Dates,'Expense for the job No'+' :'+jm_job.Number +', client: '+ mst_client.name as Descriptions,0 as Debit,jm_expensemaster.GrandTotal as Credit from jm_expensemaster
    ...
    {
        $postdata="select jm_expensemaster.ExpenseMasterId as id,jm_expensedetail.Description as particulars,'DE'+jm_expensemaster.PostId as doc,jm_expensemaster.Reference+'|'+jm_expensemaster.OurInv as refinv, convert(jm_expensemaster.InvDate,date)as Dates,jm_expensemaster.GrandTotal as amount,mst_supplier.supplier_name as suppliername from jm_expensemaster
